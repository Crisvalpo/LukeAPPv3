-- Fix functions broken by subscription_plans.id type change (Text -> Enum)
-- Must DROP first to avoid "cannot change return type" error
...
-- 1. get_company_subscription_info
...
BEGIN
    ...
    -- JOIN updated: Both are now ENUM 'subscription_tier', so no cast needed
...
BEGIN
    ...
    -- Obtener límite del plan (Fixed Join)
