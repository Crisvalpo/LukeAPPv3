...
export async function createRevision(
    ...
    try {
        ...
        const eventResult = await emitEvent({
            ...
            event_type: 'CREATED' as RevisionEventTypeEnum,
            payload: {
                rev_id: params.rev_id,
                entity_type: params.entity_type,
                entity_id: params.entity_id
        ...
        const revisionsWithData = await Promise.all(
            revisions.map(async (rev: any) => {
                ...
                return {
                    ...
                    transmittal_code: rev.transmittal_code,
                    transmittal_date: rev.transmittal_date,
                    release_date: rev.release_date,
                    announcement_date: rev.announcement_date,
                    has_production_data: rev.has_production_data,
                    spools_loaded: rev.spools_loaded,
                    welds_loaded: rev.welds_loaded,
                    mto_loaded: rev.mto_loaded,
                    bolted_joints_loaded: rev.bolted_joints_loaded,
                    data_status: rev.data_status,
                    material_status: rev.material_status,
                    requires_joints: rev.requires_joints,
