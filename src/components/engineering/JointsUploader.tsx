...
import { Upload, Bolt, CheckCircle2, Trash2, RefreshCcw, AlertCircle, Loader2 } from 'lucide-react'
                ...
                <div className="bg-black/20 border-2 border-dashed border-glass-border/30 rounded-xl p-8 hover:bg-black/30 transition-all group relative">
                    {!file ? (
                        <div className="flex flex-col items-center justify-center text-center">
                            <div className="w-16 h-16 rounded-full bg-brand-secondary/20 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-inner border border-brand-secondary/30">
                                <Bolt size={32} className="text-brand-secondary opacity-80" />
                            </div>
                            <h3 className="text-lg font-semibold text-white mb-2">Cargar Reporte de Juntas</h3>
                            <p className="text-text-dim text-sm max-w-sm mb-6">
                                Arrastra y suelta tu archivo Excel (Bolted Joints) aquí, o haz clic para seleccionar.
                            </p>
                            <input
                                type="file"
                                accept=".xlsx, .xls"
                            ...
                            />
                            <div className="px-6 py-2.5 bg-brand-secondary/10 border border-brand-secondary/20 text-brand-secondary rounded-lg font-medium group-hover:bg-brand-secondary/20 transition-colors pointer-events-none">
                                Seleccionar Archivo
                            </div>
                        </div>
                    ) : (
                        <div className="flex flex-col items-center animate-in fade-in zoom-in-95 duration-200">
                            <div className="w-16 h-16 rounded-xl bg-status-success/10 border border-status-success/20 flex items-center justify-center mb-4">
                                <Bolt size={32} className="text-status-success" />
                            </div>
                            ...
                            <div className="flex gap-3">
                                ...
                                <Button
                                    ...
                                    disabled={uploading}
                                    className="bg-brand-secondary text-white hover:bg-brand-secondary/90 gap-2 min-w-[140px]"
                                >
                                    {uploading ? <Loader2 size={16} className="animate-spin" /> : <Upload size={16} />}
                                    {uploading ? 'Procesando...' : 'Cargar Juntas'}
                                </Button>
                            </div>
                        </div>
                    )}
                ...
                <div className="bg-status-success/5 border border-status-success/20 rounded-xl p-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="flex flex-col items-center text-center mb-6">
                        ...
                        <p className="text-text-dim text-sm">
                            Se han procesado correctamente los datos de Juntas.
                        </p>
                    </div>
                    ...
                    {jointsCount > 0 && (
                        <div className="grid grid-cols-1 gap-4 mb-6 max-w-xs mx-auto">
                            <div className="bg-black/20 border border-glass-border/30 rounded-lg p-4 flex items-center justify-between">
                                <span className="text-text-dim text-sm">Juntas Cargadas</span>
                                <span className="text-xl font-bold text-white">{jointsCount.toLocaleString()}</span>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex justify-center gap-3">
                        ...
                        >
                            <Trash2 size={16} className="mr-2" /> Eliminar Juntas
                        </Button>
                        <Button
                            variant="outline"
